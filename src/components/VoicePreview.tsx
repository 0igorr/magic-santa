...
const VoicePreview = () => {
  ...
  return <section className="py-12 md:py-20 px-4 bg-gradient-to-b from-accent/10 to-background">
          ...
          <div className="space-y-5 md:space-y-6">
            <div>
              <div className="space-y-2">
                <Input type="text" placeholder="Escribe el nombre..." value={childName} onChange={e => setChildName(e.target.value.slice(0, 10))} className="text-base md:text-lg py-5 md:py-6 rounded-xl border-2 border-accent/30 focus:border-accent" disabled={isGenerating} maxLength={10} />
            ...
            {!isReady && !isGenerating && <Button onClick={handleGenerate} className="w-full bg-primary hover:bg-primary/90 text-white py-4 md:py-6 rounded-xl text-sm md:text-lg font-semibold flex items-center justify-center" disabled={!childName.trim()}>
                <Mic className="w-4 h-4 md:w-5 md:h-5 mr-2 flex-shrink-0" />
                <span className="truncate">Escuchar la Voz de Papá Noel</span>
              </Button>}

            {isGenerating && <div className="flex flex-col items-center justify-center py-8 space-y-4">
                <Loader2 className="w-12 h-12 text-accent animate-spin" />
                <div className="flex gap-1">
                  {[...Array(5)].map((_, i) => <motion.div key={i} className="w-2 h-16 bg-gradient-gold rounded-full" animate={{
                scaleY: [0.3, 1, 0.3]
              }} transition={{
                duration: 1,
                repeat: Infinity,
            ...
            scale: 1
                ...
                <div className="bg-gradient-magic rounded-xl p-6 border border-accent/30">
                  ...
                  <Button onClick={handlePlay} className="bg-accent hover:bg-accent/90 text-foreground font-semibold py-3 rounded-full px-[10px] mx-0 pr-[10px] pl-[10px]">
            ...
            }} variant="outline" className="w-full border-accent/30 hover:bg-accent/10">
                  Generar Otro Nombre
                </Button>
